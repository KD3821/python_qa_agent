...
GPU_CONFIG = os.environ.get("GPU_CONFIG", "a100:3")
...
def launch(config_raw: dict, data_raw: bytes, run_to_resume: str, preproc_only: bool):
    import yaml
    ...
    with (
        ...
        open(f"{run_folder}/{config['datasets'][0]['path']}", "wb") as data_file,
    ):
...
def main(
    ...
    with open(config, "r") as cfg, open(data, "rb") as dat:
